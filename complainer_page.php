...
if(isset($_POST['s'])){
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST")
    {
		...
		$query1 = "SELECT p_id FROM police WHERE location = '$location'";
		$policemen = mysqli_query($conn, $query1);
		while ($row = mysqli_fetch_array($policemen)){
			array_push($input, $row[0]);
		}
    ...
    if($var>=0)
    {
      ...
      $comp="INSERT into complaint(a_no,location,type_crime,d_o_c,description,p_id) values('$a_no','$location','$type_crime','$d_o_c','$description','$input[$rand_keys]')";
